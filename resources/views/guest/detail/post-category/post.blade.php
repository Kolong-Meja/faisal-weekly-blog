    ...
    <div class="min-h-screen bg-zinc-900">
        ...
        <section class="py-16 md:py-20">
            <div class="container mx-auto">
                <h1 class="text-center text-2xl font-bold mb-5 text-white md:text-4xl">{{ ucwords(strtolower($category_title->title)) }} Posts</h1>
                            ...
                            <ul class="p-3 space-y-3 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownRadioButton">
                                <li>
                                    <div class="flex items-center p-2 hover:bg-gray-600">
                                        <input id="default-radio-2" type="radio" value="newest" name="filter" class="w-4 h-4 text-yellow-600 bg-gray-600 border-gray-500 focus:ring-yellow-500 dark:focus:ring-yellow-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        <label for="default-radio-2" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Newest Post</label>
                                    </div>
                                </li>
                                <li>
                                    <div class="flex items-center p-2 hover:bg-gray-600">
                                        <input id="default-radio-3" type="radio" value="oldest" name="filter" class="w-4 h-4 text-yellow-600 bg-gray-600 border-gray-500 focus:ring-yellow-500 dark:focus:ring-yellow-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        <label for="default-radio-3" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Oldest Post</label>
                                    </div>
                                </li>
                            </ul>
                            <div class="px-2 flex justify-center">
                                <button type="submit" class="text-white bg-yellow-500 hover:bg-yellow-600 transition-colors duration-300 ease-in-out w-50 focus:ring-4 hover:text-gray-900 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none block">Submit</button>
                            </div>
                        </form>
                    </div> --}}
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 px-5 md:grid-cols-3 md:gap-8 md:px-0">
                    @foreach ($post_categories as $post_category)
                        <div id="post__card" class="bg-yellow-400 rounded-b-lg shadow-xl shadow-gray-900/40 overflow-hidden hover:bg-yellow-500 transition-colors duration-300 ease-in-out md:max-w-md md:h-min">
                            @foreach ($post_category->images as $image)
                                <a href="{{ $image->url }}">
                                    <img src="{{ asset('images/'.$image->image) }}" class="w-full h-64 object-cover object-center" title="Image by: {{ $image->owner }}">
                                </a>
                            @endforeach
                            <a href="{{ route('posts.show', $post_category->slug) }}" class="no-underline" title="{{ $post_category->title }}">
                                <div class="px-4 pt-4 mb-2">
                                    <h2 class="text-[22px] font-bold mb-2 text-gray-900">
                                        {{ $post_category->title }}
                                    </h2>
                                    <p class="text-[16px] text-gray-900">{{ $post_category->description }}</p>
                                </div>
                                <div class="p-4 flex justify-between items-center mt-auto content-end">
                                    <span class="text-gray-900 font-semibold text-sm">{{ Carbon\Carbon::parse($post_category->created_at)->toFormattedDateString() }}</span>
                                    <span class="text-gray-900 font-semibold text-sm">Read time: {{ $post_category->reading_duration }} Minute</span>
                                </div>
                            </a>
                        </div>
                    @endforeach
            ...
            {{ $post_categories->links('vendor.pagination.simple-tailwind') }}
        </section>  
        <footer class="py-8">

    ...
    <div class="min-h-screen bg-zinc-900">
        ...
        <div class="py-16 md:py-24">
            <div class="container mx-auto">
                <h1 class="text-center text-2xl font-bold mb-5 text-white md:text-4xl">All Categories</h1>
                @if (!$categories->isEmpty())
                    <form autocomplete="on">
                        <div class="relative mb-5 flex justify-center px-5 md:px-0">
                            ...
                            <input type="search" name="search" id="default-search" class="block w-full p-4 pl-5 text-sm bg-zinc-900 rounded-none rounded-r-lg shadow-lg border-2 border-l-0 border-gray-200 focus:border-gray-100 focus:ring-0 md:w-2/4" placeholder="Search category..." value="{{ request('search') }}">
                        </div>
                    </form>
                    <p class="text-center text-sm mb-10 px-5 text-gray-400 md:text-lg md:px-0">Note: You can search posts by it's category, all categories have posts related to them.</p>
                @else
                    <p class="text-gray-400 flex justify-center text-md">No Categories Available.</p>
                ...
                <div class="grid grid-cols-2 gap-4 px-5 md:grid-cols-5 md:gap-8 md:px-0">
                    @foreach ($categories as $category)
                        <div class="bg-yellow-400 rounded-lg shadow-xl overflow-hidden hover:bg-yellow-500 transition-colors duration-300 ease-in-out md:max-w-md">
                            <div>
                                <h2 class="text-center font-bold text-neutral-900">
                            ...
                            <div class="flex justify-center pb-2 px-2">
                                <p class="text-black text-sm text-center md:text-md">{{ $category->posts()->count() }} related posts</p>
                            </div>
                        </div>
                    @endforeach
                </div>
